...
Zarf `actions` can also pull values from the shell's environment when running a `cmd`.  These values are available under the same `ZARF_VAR_<VALUE_KEY>` as value templates (without any `#`s) and can be used like the below:
